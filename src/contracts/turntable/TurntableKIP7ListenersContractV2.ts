...
export default abstract class TurntableKIP7ListenersContractV2 extends Contract {
    ...
    public async turntableShares(turntableId: BigNumberish): Promise<BigNumber> {
        return BigNumber.from(await this.runMethod("turntableShares", turntableId));
    }
    ...
    public async claimableOf(turntableId: BigNumberish, owner: string, token: string): Promise<BigNumber> {
        return BigNumber.from(await this.runMethod("claimableOf", turntableId, owner, token));
    }
    ...
    public async claim(turntableId: BigNumberish, token: string): Promise<void> {
