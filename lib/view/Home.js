...
class Home {
    ...
    async loadDogeSound() {
        try {
            ...
            const round = (await DogeSoundContestV2Contract_1.default.getRound()).toNumber() - 1;
            const elected = (await DogeSoundContestV2Contract_1.default.getElected(round)).toNumber();
            const dogesound = await DogeSoundContestV2Contract_1.default.getCandidate(round, elected);
            const winner = await DogeSoundContestV2Contract_1.default.getCandidateRegister(round, elected);
            this.winner.empty().appendText(`${(0, msg_js_1.default)("HOME_WINNER_TITLE").replace(/{round}/, String(round + 1))} `);
            this.winner.append((0, skynode_1.el)("a", winner, { href: `https://opensea.io/${winner}` }));
            this.dogesound.empty().appendText(`${(0, msg_js_1.default)("HOME_WINNER_DESCRIPTION").replace(/{round}/, String(round + 1))} \nㅡ ${dogesound}`);
        }
        catch (e) {
