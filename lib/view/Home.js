...
class Home {
    ...
    async loadDogeSound() {
        try {
            ...
            const dogesound = await SloganContract_1.default.getCandidate(round, elected);
            const winner = await SloganContract_1.default.getCandidateRegister(round, elected);
            this.winner.appendText(`${(0, msg_js_1.default)("HOME_WINNER_TITLE").replace(/{round}/, String(round + 1))} `);
            this.winner.append((0, skynode_1.el)("a", winner, { href: `https://opensea.io/${winner}` }));
            this.dogesound.appendText(`${(0, msg_js_1.default)("HOME_WINNER_DESCRIPTION").replace(/{round}/, String(round + 1))} \nㅡ ${dogesound}`);
        }
        catch (e) {
