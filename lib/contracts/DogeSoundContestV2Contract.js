...
class DogeSoundContestContractV2 extends Contract_1.default {
    ...
    async getCheckpoint() {
        return ethers_1.BigNumber.from(await this.contract.methods.checkpoint().call());
    }
    async getRound() {
    ...
    async getMateVoted(round, mates, candidate) {
        return await this.contract.methods.mateVoted(round, mates, candidate).call();
    }
    async registerCandidate(dogeSound, mates, count) {
    ...
    async vote(candidate, mates, count) {
